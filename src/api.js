...
module.exports = function (projectName, environment, build, fixtureName, stepName, status, durationMs) {
    try {
        console.log(url + '*******************************');
        ...
        axios.post(url, { 'projectName': projectName, 'environment': environment, 'build': build, 'fixtureName': fixtureName, 'stepName': stepName, 'status': status, 'durationMs': durationMs })
            .catch(err => console.log('Not able to post test through api', err));
    }
    catch (error) {
